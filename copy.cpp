...
void merge_nodes_recursive(Node** nodes, int* num_nodes_ptr, int init, int max_len) {
    printf("%d %d\n", (*num_nodes_ptr), max_len);
    while ((*num_nodes_ptr) - init > 1) { // 确保有至少两个节点可以合并
        qsort(&nodes[init], (*num_nodes_ptr) - init, sizeof(Node*), compare_nodes);
        // for (int i = 0; i < (*num_nodes_ptr); i++) {
        //     printf("%d ", nodes[i]->depth);
        // }
        // printf("\n");

        Node* smallest = nodes[init];
        Node* second_smallest = nodes[init + 1];

        // 合并最小的两个节点
        Node* left = smallest;
        Node* right = second_smallest;
        if (left->depth + 1 < max_len && right->depth + 1 < max_len) {
            ...
            new_node->symbol = -1; // 内部节点
            new_node->left = left;
            new_node->right = right;
            new_node->depth = (left->depth > right->depth ? left->depth : right->depth) + 1;

            // 用新节点替换最小的节点
            nodes[init] = new_node;
            // 将第二小的节点替换为最后一个节点
            nodes[init + 1] = nodes[(*num_nodes_ptr) - 1];
            (*num_nodes_ptr)--;
        } else if (left->depth + 1 < max_len) {
            swap(&nodes[init], &nodes[init + 1]);
        }
        // printf("%d \n", nodes[init]->depth + 1);

        // 继续合并
        if (nodes[init]->depth + 1 == max_len) {
            for (int i = 0; i < (*num_nodes_ptr); i++) {
                printf("%d ", nodes[i]->depth);
            }
            printf("\n");
            merge_nodes_recursive(nodes, num_nodes_ptr, init + 1, max_len - 1);
            // 合并最小的两个节点
            Node* left = nodes[init];
            Node* right = nodes[init + 1];
            Node* new_node = (Node*)malloc(sizeof(Node));
            if (!new_node) {
                fprintf(stderr, "Memory allocation failed\n");
                exit(EXIT_FAILURE);
...
void assign_codes_from_tree(Node* node, unsigned int code, int code_length, Symbol symbols[], int num_symbols) {
    if (!node->left && !node->right) {
        // 如果是叶节点，找到对应符号并分配编码和编码长度
        for (int i = 0; i < num_symbols; i++) {
            if (symbols[i].symbol == node->symbol) {
                symbols[i].code = code;
                symbols[i].code_length = code_length;
                break;
            }
        }
        return;
    ...
    // 遍历左子树，左边添加0
    if (node->left) {
        assign_codes_from_tree(node->left, code << 1, code_length + 1, symbols, num_symbols);
    }

    // 遍历右子树，右边添加1
    if (node->right) {
        assign_codes_from_tree(node->right, (code << 1) | 1, code_length + 1, symbols, num_symbols);
    }
}
...
void generate_optimal_huffman_table(const long freq[257], JHUFF_TBL* htbl) {
    ...
    // Step 3: Build the Huffman tree with depth constraint
    ...
    for (int i = 0; i < num_symbols; i++) {
        ...
        nodes[i]->depth = 0; // Leaf nodes have depth 0
    }

    int num_nodes = num_symbols;
    merge_nodes_recursive(nodes, &num_nodes, 0, MAX_CLEN);

    // The remaining node is the root of the Huffman tree
    Node *root = nodes[0];

    // Step 4: Assign codes by traversing the Huffman tree
    assign_codes_from_tree(root, 0, 0, symbols, num_symbols);

    // Step 5: Sort symbols by code length and symbol using the provided compare function
    qsort(symbols, num_symbols, sizeof(Symbol), compare_symbols_by_length_and_symbol);

    // Step 6: Build the Huffman table
    memset(htbl->bits, 0, sizeof(htbl->bits));
    memset(htbl->huffval, 0, sizeof(htbl->huffval));
    ...
    // Step 7: Print out the code values for each symbol
